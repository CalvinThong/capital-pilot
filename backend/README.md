...
Set `REGIME_REGISTRY_ADDRESS` to a deployed `MarketRegimeRegistry` to record each resolved LLM_A regime on-chain when execution is enabled. The registry stores an append-only array containing the regime, confidence, block timestamp, and reason. Backend consumers can call `TradingClient.readMarketRegimeHistory(offset, limit)` to retrieve up to 100 records at a time; each record includes both `recordedAt` Unix time and `recordedAtIso`.
