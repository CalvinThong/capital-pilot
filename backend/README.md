...
Set `REGIME_REGISTRY_ADDRESS` to the `MarketRegimeRegistry` deployed with the active factory. When execution is enabled, each scan stores one combined LLM_A and LLM_B decision containing the regime, selected-strategy mask, confidence values, timestamp, and rationales. Every successful open and close stores that decision ID, its action-specific confidence, and reason in the vault's position history. Backend consumers can call `TradingClient.readMarketRegimeHistory(offset, limit)` to retrieve up to 100 market decisions at a time.
